...
class SQLiteDatabase(Database):
	...
	def getWordFilter(self, filtername):
		sql = "SELECT * FROM wordfilters  WHERE name=?;"
		with self.lock:
	...
	def setWordFilter(self, filtername, badword, replacement):
		sql = "INSERT INTO wordfilters VALUES (?,?,?) ON CONFLICT(name) DO UPDATE SET badword=?, replacement=?;"
		with self.lock:
			self.db.execute(sql, (filtername, badword, replacement, badword, replacement))
	def removeWordFilter(self, filtername):
		sql = "DELETE FROM wordfilters WHERE name=?;"
		with self.lock:
